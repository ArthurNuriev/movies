...
import {genreAdventureService} from "../../../services/genreAdventureService";
...
const AdventureList = () => {
    ...
    const [path] = useState(URLS.genreAdventure);
    const [page, setPage] = useState(numOfPage.page);
    const [total_pages, setTotal_pages] = useState(500);


    useEffect(() => {
        genreAdventureService.getPage(numOfPage.page).then(({data}) => {
