...
import {genreAnimationService} from "../../../services/genreAnimationService";
...
const AnimationList = () => {
    ...
    const [path] = useState(URLS.genreAnimation);
    const [page, setPage] = useState(numOfPage.page);
    const [total_pages, setTotal_pages] = useState(500);


    useEffect(() => {
        genreAnimationService.getPage(numOfPage.page).then(({data}) => {
